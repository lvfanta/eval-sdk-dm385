    ...
    if [ ! -f $DIR/$PKG ] ; then
        case $PKG_TYPE in
            ...
            git )
                ...
                tar $VERBOSE_ARGS --exclude=.git -cjf $DIR/$PKG . || { echo "Failed to create archive: $DIR/$PKG" ; exit 255; }
            ...
            svn )
                ...
                #Create the package tarball
                tar $VERBOSE_ARGS --exclude=.svn -cjf $DIR/$PKG . || { echo "Failed to create archive: $DIR/$PKG" ; exit 255; }
                #Jump to previous directory
                popd >/dev/null
                #Remove temporal directory
                rm -Rf /tmp/rrsdk_$$/$PKG_NAME
