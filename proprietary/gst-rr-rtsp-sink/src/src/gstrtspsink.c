...
static gboolean gst_rtsp_sink_set_caps(GstPad *pad, GstCaps *caps)
{
  ...
  sink->pipeline = g_malloc (strlen(PIPEPROLOG) +
      strlen(PIPEEPILOG) + strlen("rtph264pay buffer-list=true")+1);
  ...
  if (!strcmp (name, "video/x-h264")) {
    sprintf(sink->pipeline, "%srtph264pay buffer-list=true%s", PIPEPROLOG, PIPEEPILOG);
  ...
  } else if (!strcmp (name, "video/mpegts")) {
    sprintf(sink->pipeline, "%srtpmp2tpay%s", PIPEPROLOG, PIPEEPILOG);
  } else {
